...
void ArgumentList::verifySyntaxCreateParseTree(int tokenCounter, ParseTreeNode* motherNode)
{
	   ...
	   )
	{
		...
		if (!isValid)
		{
			...
			//Now, we have an empty ArgumentList_ object, so we return to prevent infinite recursion.
			return;
		}


		//Oh Yes!  We had better return or else we will get infinite recursion again!
		return;
	}
